    ...
    else //valid action provided
    {
        ...
        else if (action == "getlong") //getLong action
        {
            ...
            if (Uri.IsWellFormedUriString(url, UriKind.Absolute))//Checking if tinyURL provided is a valid format for Uri
            {
                ...
                if(longURL == "err") //Service returns err when tinyURL is not associated with long URL
                {
                    Console.ForegroundColor = ConsoleColor.Red;
                    Console.WriteLine("Error, this tinyURL is not associated with a URL");
                }
                else
                {
                    Console.ForegroundColor = ConsoleColor.Green;
        ...
        else if (action == "getstats") //getStats action
        {
            ...
            if (Uri.IsWellFormedUriString(url, UriKind.Absolute))//Checking if tinyURL provided is a valid format for Uri
            {
                if (count == -1) //Service returns -1 when an error has occurred
                {
                    Console.ForegroundColor = ConsoleColor.Red;
                    Console.WriteLine("Oops, an error has occurred. This tinyURl does not exist.");
